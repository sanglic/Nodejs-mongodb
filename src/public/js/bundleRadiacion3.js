...
var svg = d3.select("#grafica")
  ...
  .append("g")
    ...
    svg.append("text")
        ...
        .text("SW_in_Avg - SW_out_Avg - LW_in_Avg - LW_out_Avg - Rnet_Avg")
        .style("color","#008B8B");
    ...
    .attr("y", (20+height+margin.bottom/2))
    .text("LW_in_Avg").style("font-size", "15px")
    .attr("alignment-baseline","middle")
    .style("stroke", "blue")
    .attr("stroke-width","0.5")
    .attr("opacity", 0.7)

    svg.append("text").attr("x", (159+width / 2))
    .attr("y", (20+height+margin.bottom/2))
    .text("LW_out_Avg").style("font-size", "15px")
    .style("stroke", "brown")
    .attr("stroke-width","0.5")
    .attr("alignment-baseline","middle")
    .attr("opacity", 0.7)
    ...
    .attr("y", (20+height+margin.bottom/2))
    .text("Rnet_Avg").style("font-size", "15px")
    .attr("alignment-baseline","middle")
    .style("stroke", "grey")
    .attr("stroke-width","1")
    .attr("opacity", 0.7)
...
d3.csv("./data/Radiacion.csv",
  ...
  function(d){
    return { date : d3.timeParse("%Y-%m-%d %H:%M:%S")(d.date), SW_in_Avg : Number(d.SW_in_Avg), SW_out_Avg: Number(d.SW_out_Avg),LW_in_Avg : Number(d.LW_in_Avg), LW_out_Avg: Number(d.LW_out_Avg), Rnet_Avg: Number(d.Rnet_Avg)}
  ...
  */
  ...
  // Add w axis
   ...
   // Add k axis
    var k = d3.scaleLinear()
      .domain([0, d3.max(data, function(d) { return +d.Rnet_Avg; })])
      .range([ height, 0 ]);
    var wkxis = svg.append("g")
      .attr("class", "axisBlue")
      .call(d3.axisLeft(v));
  ...
  // Add a clipPath: everything out of this area won't be drawn.
    ...
    var clip3 = svg.append("defs").append("svg:clipPath") //PRUEBA
        .attr("id", "clip3")
        .append("svg:rect")
        .attr("width", width )
        .attr("height", height )
    ...
    var clip5 = svg.append("defs").append("svg:clipPath") //PRUEBA
        .attr("id", "clip5")
        .append("svg:rect")
        .attr("width", width )
        .attr("height", height )
 ...
 // add the Y gridlines
    ...
    var line3=svg.append("g") ///prueba
      .attr("clip-path","url(#clip3)" )

    var line4=svg.append("g") ///prueba
      .attr("clip-path","url(#clip4)" )

    var line5=svg.append("g") ///prueba
    ...
    line2.append("path") ///PRUEBA
      ...
      .attr("d", d3.line()
        .x(function(d) { return x(d.date) })
        .y(function(d) { return w(d.LW_in_Avg) })
        )
      ...
      .attr("d", d3.line()
        .x(function(d) { return x(d.date) })
        .y(function(d) { return v(d.LW_out_Avg) })
        )
      ...
      .attr("d", d3.line()
        .x(function(d) { return x(d.date) })
        .y(function(d) { return k(d.Rnet_Avg) })
        )
        // Add the brushing
    line
      .append("g")
    ...
    svg.on("dblclick",function(){
      ...
      line4
        ...
        .attr("d", d3.line()
          .x(function(d) { return x(d.date) })
          .y(function(d) { return v(d.Rnet_Avg) }),
          Info_points()  
	 ...
	 line2.selectAll("dot")  //Selecccionamos los puntos de la linea
        ...
        .on("mouseover", function(d) {                         //añadimos la cajita cada vez que pasamos el ratón
            div.transition()    
                .duration(200)    
                .style("opacity", 1);    
            div .html((d.date) + "<br/>"+"SW_in_Avg: "+d.SW_in_Avg+"         "+"SW_out_Avg: " + d.SW_out_Avg+"<br/>"+"LW_in_Avg: "+d.LW_in_Avg+"         "+"LW_out_Avg: " + d.LW_out_Avg+"   Rnet_avg: " + d.Rnet_Avg)  
...
line3.selectAll("dot")  //Selecccionamos los puntos de la linea
        .data(data)     
        .enter().append("circle")
        .style("opacity",0)             
        .attr("r", 3)   
        .attr("cx", function(d) { return x(d.date); })     
        .attr("cy", function(d) { return w(d.LW_in_Avg);})   
        .on("mouseover", function(d) {                         //añadimos la cajita cada vez que pasamos el ratón
            div.transition()    
                .duration(200)    
                .style("opacity", 1);    
...
line4.selectAll("dot")  //Selecccionamos los puntos de la linea
        .data(data)     
        .enter().append("circle")
        .style("opacity",0)             
        .attr("r", 3)   
        .attr("cx", function(d) { return x(d.date); })     
        .attr("cy", function(d) { return v(d.LW_out_Avg);})   
        .on("mouseover", function(d) {                         //añadimos la cajita cada vez que pasamos el ratón
            div.transition()    
                .duration(200)    
                .style("opacity", 1);     
   ...
   line5.selectAll("dot")  //Selecccionamos los puntos de la linea
        .data(data)     
        .enter().append("circle")
        .style("opacity",0)             
        .attr("r", 3)   
        .attr("cx", function(d) { return x(d.date); })     
        .attr("cy", function(d) { return k(d.Rnet_Avg);})   
        .on("mouseover", function(d) {                         //añadimos la cajita cada vez que pasamos el ratón
            div.transition()    
                .duration(200)    
                .style("opacity", 1);     
        ...
        .on("mouseover", function(d) {                         //añadimos la cajita cada vez que pasamos el ratón
            div.transition()    
                .duration(200)    
                .style("opacity", 1);    
            div .html((d.date) + "<br/>"+"SW_in_Avg: "+d.SW_in_Avg+"         "+"SW_out_Avg: " + d.SW_out_Avg+"<br/>"+"LW_in_Avg: "+d.LW_in_Avg+"         "+"LW_out_Avg: " + d.LW_out_Avg)  
                .style("left", (d3.event.pageX-100)+ "px")   
                .style("top", (d3.event.pageY - 100) + "px")
                .style("color","#008B8B")
                .style("opacity", 1);
