...
var svg = d3.select("#grafica2")
  ...
  .append("g")
    ...
    svg.append("text")
        .attr("x", (width / 2))             
        .attr("y", (height+margin.bottom/2))
        .attr("transform", "translate(-325 ,290) rotate(270)")
        .attr("text-anchor", "middle")  
    ...
    .attr("y", (20+height+margin.bottom/2))
    .text("SW_out").style("font-size", "15px")
    .attr("alignment-baseline","middle")
    .style("stroke", "red")
    .attr("stroke-width","0.5")
    .attr("opacity", 0.7)
...
d3.csv("./data/"+valor_csv,
  ...
  function(d){
    return { SW_in_Avg: Number(d.SW_in_Avg), SW_out_Avg: Number(d.SW_out_Avg), 
    SW_in_HQ_Avg: Number(d.SW_in_HQ_Avg)}
	 ...
	 line2.selectAll("dot")  //Selecccionamos los puntos de la linea
        ...
        .on("mouseover", function(d) {                         //añadimos la cajita cada vez que pasamos el ratón
            div.transition()    
                .duration(200)    
                .style("opacity", 1);  
            div .html("SW_in_HQ: "+d.SW_in_HQ_Avg + " Wm-2" + "<br/>"+"SW_in: "+d.SW_in_Avg+" Wm-2"+"<br/>"+"SW_out: " + d.SW_out_Avg+" Wm-2")  
